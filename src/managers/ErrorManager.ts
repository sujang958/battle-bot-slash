import Discord, { Message, CommandInteraction, TextChannel } from 'discord.js'
...
export default class ErrorManager extends BaseManager {
	...
	public report(error: Error, user: ErrorExecuter) {
		...
		const thisGuild = this.client.guilds.cache.get(user.executer.guildId)
		...
		if(config.report.type == 'webhook') {
			...
			const channel = guild.channels.cache.get(config.report.text.channelID) as TextChannel
